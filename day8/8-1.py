...
nodes = dict([tuple([foundItems[0] if len(foundItems:=re.findall(r"(\w+)",nodeItem)) == 1 else tuple(foundItems) for nodeItem in nodeLine.split('=')]) for nodeLine in taskInput.split('\n')[2::]])
...
while currentNode != 'ZZZ':
    ...
    print(f"re-applying moveset as finished at {currentNode}, step count {totalSteps}")
